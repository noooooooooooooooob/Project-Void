...
| 6.2 카드 아래 `근접 · 사거리 2 · 단일` 한 줄 | (최종 리뷰 수정, 2026-09-14) 왼쪽 위 정렬 두 줄 `사거리 2
근접 · 단일`, (8, 114) 78×34 | 한 줄이 카드 밖으로 넘치고 부채꼴에서 오른쪽 약 20px 가 다음 카드에 가려져 사거리가 안 보였다 |
| 6.2 SP 부족은 흐림만 | (최종 리뷰 수정, 2026-09-14) 비용이 SP 를 넘는 카드는 누르기·드래그 무시 | 예전 버튼 손패는 비활성이었는데 흐린 카드가 선택·드래그되어 규칙이 거절하는 퇴행이 있었다 |
| 6.4 `set_cards` 는 항상 다시 만듦 | (최종 리뷰 수정, 2026-09-14) 같은 손패면 뷰 유지, 선택·SP 만 갱신 | 재생 직후 동기화가 날아오는 마지막 카드를 끊었다 |
| 6.4 조준 화살표·들린 카드 순서 | (최종 리뷰 수정, 2026-09-14) 화살표 `z_index = 100`, 배치 끝에 카드 자식 순서를 인덱스 순서로 맞춘 뒤 선택 카드, 화살표를 맨 뒤로 | 화살표가 끄는 카드 밑에 그려지고, 들린 카드가 겹친 오른쪽 이웃에게 클릭을 뺏겼다. 선택 카드만 옮기면 선택이 풀린 뒤 그 카드가 위에 그려진 오른쪽 이웃의 클릭을 가로챘다 |
| 6.6 로그 패널 왼쪽 위 16..376 × 56..216 | (최종 리뷰 수정, 2026-09-14) 16..216 × 56..380 | 아군 유닛(사수, 정찰병 이름)을 가렸다 |
